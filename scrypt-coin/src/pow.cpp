...
/**
 * Check AdaptivePow proof of work
 * This is the new GPU-mineable algorithm
 */
bool CheckAdaptiveProofOfWork(
    const uint256& headerHash,
    uint64_t nNonce,
    const uint256& mixHash,
    unsigned int nBits,
    int64_t nTime,
    int64_t nGenesisTime,
    const Consensus::Params& params)
{
    ...
    uint32_t epoch = GetAdaptivePowEpoch(nTime, nGenesisTime, params);

    // Verify using AdaptivePow
    uint256 target = bnTarget.getuint256();
    if (!CheckAdaptivePow(headerHash, nNonce, mixHash, target, epoch, params, g_adaptivePowDAG.get())) {
        return error("CheckAdaptiveProofOfWork() : AdaptivePow verification failed");
    }
