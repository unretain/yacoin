...
#ifndef SCRYPT_PRIMITIVES_BLOCK_H
#define SCRYPT_PRIMITIVES_BLOCK_H
...
static const int
    ...
    VERSION_of_block_for_scrypt_adaptivepow = 8,  // AdaptivePow with 64-bit nonce and mixHash
    CURRENT_VERSION_of_block = VERSION_of_block_for_scrypt_adaptivepow;
...
public:
    ...
    // AdaptivePow fields (version >= 8)
    ::uint64_t nNonce64;    // 64-bit nonce for GPU mining
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        // AdaptivePow fields for version >= 8
        if (this->nVersion >= VERSION_of_block_for_scrypt_adaptivepow) {
            READWRITE(nNonce64);
            READWRITE(hashMix);
        }
    ...
    // Get header hash for AdaptivePow (excludes nNonce64 and hashMix)
    uint256 GetAdaptivePowHeaderHash() const
    {
        CHashWriter ss(SER_GETHASH, PROTOCOL_VERSION);
        ss << nVersion << hashPrevBlock << hashMerkleRoot << nTime << nBits;
        return ss.GetHash();
    }

    // Check if this block uses AdaptivePow
    bool IsAdaptivePow() const
    {
        return nVersion >= VERSION_of_block_for_scrypt_adaptivepow;
    }
...
#endif // SCRYPT_PRIMITIVES_BLOCK_H
