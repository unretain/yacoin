...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
  ...
  // For AdaptivePow blocks, verify using the AdaptivePow algorithm
  if (pblock->IsAdaptivePow()) {
    ...
    if (!CheckAdaptivePow(headerHash, pblock->nNonce64, pblock->hashMix, hashTarget,
                          epoch, Params().GetConsensus(), g_adaptivePowDAG.get())) {
      return error("CheckWork () : AdaptivePow proof-of-work not valid");
    }
...
//_____________________________________________________________________________
static void ScryptMiner() // AdaptivePow miner for Scrypt Coin
{
    LogPrintf("ScryptMiner -- started (AdaptivePow)\n");
    ...
    try {
        ...
        while (fGenerateYacoins && nBlocksToGenerate != 0) {
            ...
            LogPrintf("Running ScryptMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize( * pblock, SER_NETWORK, PROTOCOL_VERSION));

            //
            // Initialize AdaptivePow DAG for this epoch
            //
            uint32_t epoch = GetAdaptivePowEpoch(pblock->nTime, Params().GenesisBlock().nTime, Params().GetConsensus());
            if (!g_adaptivePowDAG || !g_adaptivePowDAG->IsValid(epoch)) {
                LogPrintf("ScryptMiner: Initializing DAG for epoch %u...\n", epoch);
                if (!InitAdaptivePowDAG(epoch, Params().GetConsensus())) {
                    LogPrintf("ScryptMiner: Failed to initialize DAG!\n");
                    break;
                }
            }

            //
            // Search using AdaptivePow
            //
            ...
            // Get header hash for AdaptivePow (excludes nonce64 and mixHash)
            uint256 headerHash = pblock->GetAdaptivePowHeaderHash();

            while (fGenerateYacoins && nBlocksToGenerate != 0) {
                // Compute AdaptivePow hash with current nonce
                AdaptivePowResult result = AdaptivePowHash(headerHash, pblock->nNonce64, g_adaptivePowDAG.get(), Params().GetConsensus());

                if (result.hashFinal <= hashTarget) { // Found a solution
                    // Set the mix hash in the block
                    pblock->hashMix = result.hashMix;

                    LogPrintf("target: %s\n", hashTarget.ToString());
                    LogPrintf("result: %s\n", result.hashFinal.ToString());
                    ...
                    if (CheckWork(pblock, * pWallet, reservekey)) {
                        LogPrintf("ScryptMiner:\n  AdaptivePow proof-of-work found\n  hash: %s\n  target: %s\n", result.hashFinal.GetHex(), hashTarget.GetHex());
                ...
                // 64-bit nonce won't overflow in practice, but check periodically
                if ((pblock->nNonce64 & 0xFFFFFFFF) == 0xFFFF0000)
                    break;
    ...
    {
        LogPrintf("ScryptMiner -- terminated\n");
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ScryptMiner -- runtime error: %s\n", e.what());
    }

    LogPrintf("ScryptMiner stopped for proof-of-work\n");
}

int GenerateScrypt(bool fGenerate, int nThreads, int nblocks)
{
    ...
    LogPrintf("Starting %d ScryptMiner (AdaptivePow) thread%s\n", nThreads, (1 < nThreads) ? "s" : "");
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(&ScryptMiner);
    }
...
int GenerateYacoins(bool fGenerate, int nThreads, int nblocks) {
    return GenerateScrypt(fGenerate, nThreads, nblocks);
}
