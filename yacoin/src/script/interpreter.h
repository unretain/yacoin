...
#ifndef YACOIN_SCRIPT_INTERPRETER_H
#define YACOIN_SCRIPT_INTERPRETER_H
...
enum
{
    ...
    SCRIPT_VERIFY_DERSIG    = (1U << 2), // NEW

    // Passing a non-strict-DER signature or one with S > order/2 to a checksig operation causes script failure
    // (softfork safe, BIP62 rule 5).
    SCRIPT_VERIFY_LOW_S     = (1U << 3), // enforce low S values in signatures (depends on STRICTENC)

    // verify dummy stack item consumed by CHECKMULTISIG is of zero-length (softfork safe, BIP62 rule 7).
    SCRIPT_VERIFY_NULLDUMMY = (1U << 4), // verify dummy stack item consumed by CHECKMULTISIG is of zero-length

    // Using a non-push operator in the scriptSig causes script failure (softfork safe, BIP62 rule 2).
    SCRIPT_VERIFY_SIGPUSHONLY = (1U << 5), // NEW
    ...
    // (softfork safe)
    SCRIPT_VERIFY_MINIMALDATA = (1U << 6), // NEW
    ...
    SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS  = (1U << 7), // NEW
    ...
    SCRIPT_VERIFY_CLEANSTACK = (1U << 8), // NEW
...
#endif // YACOIN_SCRIPT_INTERPRETER_H
